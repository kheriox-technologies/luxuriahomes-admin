...
type Note = Doc<'projectOrderNotes'>;
interface PickedImage {
	mimeType: string;
	uri: string;
}

export interface OrderNotesSheetHandle {
...
export function OrderNotesSheet({ ref }: { ref?: Ref<OrderNotesSheetHandle> }) {
	...
	const appendNote = useMutation(api.projectOrders.appendNote.appendNote);
	const deleteNote = useMutation(api.projectOrders.deleteNote.deleteNote);
	...
	const uploadImage = async (image: PickedImage): Promise<string> => {
		...
		const { uploadUrl, s3Key } = await generateUploadUrl({
			projectId: order?.projectId as ProjectOrder['projectId'],
			contentType,
			ext: 'jpg',
		});
