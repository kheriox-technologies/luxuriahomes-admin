...
export interface OrderEmailSeed {
	filename: string;
	relatedId: string;
	s3Key: string;
	subject: string;
...
export interface OrderEmailSheetHandle {
	present: (seed: OrderEmailSeed) => void;
}

const recipientSeparators = /[\s,;]+/;
const DEFAULT_SUBJECT = 'Purchase Order';

export function OrderEmailSheet({
	projectId,
	ref,
}: {
	projectId: Id<'projects'>;
	ref?: Ref<OrderEmailSheetHandle>;
}) {
	...
	const [seed, setSeed] = useState<OrderEmailSeed | null>(null);
	...
	const handleSend = async () => {
		...
		try {
			await sendEmail({
				to: recipients,
				subject: subject.trim() || seed.subject,
