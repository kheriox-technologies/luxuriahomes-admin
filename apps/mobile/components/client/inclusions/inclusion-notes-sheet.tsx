...
type Note = Doc<'projectInclusionNotes'>;
interface PickedImage {
	mimeType: string;
	uri: string;
}

export interface ClientInclusionNotesSheetHandle {
	present: (inclusion: ClientInclusion) => void;
}
...
function NoteImageThumb({
	...
	const { uri } = useClientSignedUrl(projectId, s3Key);
	return (
		<Pressable
...
export function ClientInclusionNotesSheet({
	projectId,
	ref,
}: {
	projectId: Id<'projects'>;
	ref?: Ref<ClientInclusionNotesSheetHandle>;
}) {
	...
	// Clients may only delete notes they authored (server-enforced); mirror that
	// gate in the UI. Must match `clientDisplayName` on the backend.
	const currentClientName =
		user?.fullName?.trim() ||
		user?.primaryEmailAddress?.emailAddress?.trim() ||
		'Client';

	const [inclusion, setInclusion] = useState<ClientInclusion | null>(null);
	...
	const generateUploadUrl = useAction(
		api.clientPortal.inclusions.generateNoteImageUploadUrl
			.generateNoteImageUploadUrl
	);

	const notes = useQuery(
		api.clientPortal.inclusions.listNotes.listNotes,
		inclusion ? { projectInclusionId: inclusion._id } : 'skip'
	) as Note[] | undefined;
			...
			>
				...
				>
					<Text
						...
						numberOfLines={1}
					>
						Notes{inclusion ? ` · ${inclusion.title}` : ''}
					</Text>
						...
						{notes?.map((note) => (
							...
							>
								<View className="flex-row items-center justify-between gap-2">
									...
									{note.addedBy === currentClientName ? (
										<Pressable
