...
import { CheckCheck, CircleDashed, Download } from 'lucide-react-native';
...
import type { ClientInclusionSection } from './types';
...
export function ClientInclusionSectionHeader({
	section,
}: {
	section: ClientInclusionSection;
}) {
	...
	const runBulkStatus = async () => {
		...
		try {
			await Promise.all(
				toChange.map((i) =>
					setStatus({ projectInclusionId: i._id, status: targetStatus })
				)
			);
		} catch {
