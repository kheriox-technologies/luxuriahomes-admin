...
interface SingleFileUploadProps {
  ...
  disabled?: boolean;
  uploading?: boolean;
  /** Name of the already-uploaded file (shown as the uploaded state). */
  uploadedFileName?: string;
  onFileSelected: (file: File) => void | Promise<void>;
  /** Clear uploaded file (parent may async). */
...
export function SingleFileUpload({
  ...
  const hasUploaded = Boolean(uploadedFileName) && !uploading;
  ...
  const onDragOver = (event: DragEvent<HTMLDivElement>) => {
    ...
    if (!disabled && !uploading && !hasUploaded) {
      setIsDragging(true);
    }
  };
  ...
  const showProgressRow = Boolean(fileMeta && (uploading || !uploadedFileName));
  ...
  return (
    <Field className="w-full min-w-0 items-stretch">
      ...
      {hasUploaded ? (
        <div className="flex w-full min-w-0 items-center gap-3 rounded-lg border border-input bg-background px-3 py-2.5 shadow-xs/5">
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {onClear && !uploading && !uploadedFileName && fileMeta ? (
                      <button
                        aria-label="Remove file"
                  ...
                  {!uploading && !uploadedFileName && fileMeta ? (
                    <button
