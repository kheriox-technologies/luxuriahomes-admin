	...
	handler: async (ctx, args): Promise<void> => {
		...
		if (newPath === folder.path) {
			await ctx.runMutation(
				internal.companyDocuments.shared.updateFolderRecord,
				{
					folderId: args.folderId,
					name: trimmedName,
					path: folder.path,
		...
		for (const doc of allDocuments) {
			...
			const newS3Key = `company/documents/${prefix}${doc.kebabName}`;
		...
		if (docUpdates.length > 0) {
			await ctx.runMutation(
				internal.companyDocuments.shared.batchUpdateDocumentFolderPath,
				{ updates: docUpdates }
			);
		}
		...
		await ctx.runMutation(internal.companyDocuments.shared.updateFolderRecord, {
			folderId: args.folderId,
			name: trimmedName,
			path: newPath,
			parentPath: folder.parentPath,
