...
export const migrateImageBinariesToS3 = internalAction({
	...
	handler: async (
		...
		const records = (await ctx.runQuery(
			internal.projectInclusions.migrateImageHelpers.getPendingBatch,
			{ limit: limit + 1 }
		)) as PendingRecord[];
		...
		for (const record of batch) {
			...
			try {
				...
				await ctx.runMutation(
					internal.projectInclusions.migrateImageHelpers.patchImage,
					{ id: record._id, image: s3Key }
				);

				console.log(`migrated project inclusion ${record._id} → ${s3Key}`);
				processed++;
			} catch (error) {
				console.error(
					`skipped project inclusion ${record._id} (${imageUrl}):`,
					error
				);
				skipped++;
			}
