...
export default defineSchema({
	...
	projectDocuments: defineTable({
		...
		// Set when this PDF was generated by the in-app letter composer. Holds the
		// rich-text source (HTML) so the document can offer a "Create New Letter"
		// action that reuses its content.
	...
	// reconciled each run. With the 1:1 trade↔code model each trade's Actual is its
	// code's amount here. Amounts are uplifted (×1.1) and cents-rounded; zero/absent
	// amounts are not stored (UI shows "—").
